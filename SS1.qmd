...
library(pacman)
p_load(tidyverse, haven, broom, skimr)
...
res <- general |> 
       mutate(impacto = factor(case_when(
                     importancia < mean(importancia) & desempeño > 5 ~ 1,  
                     importancia < mean(importancia) & desempeño <= 5 ~ 2,
                     importancia >=  mean(importancia) & desempeño <= 5 ~ 3,
                     importancia >=  mean(importancia) & desempeño >  5 ~ 4),
                     levels = 1:4, labels = c("Mantener", "Mejorar en el largo plazo",
                                              "Accionar de inmediato", "Explotar")))
```
